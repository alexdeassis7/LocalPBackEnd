    ...
    public class PayOutBrasil
    {
        ...
        public class Create
        {
            public class Request
            {
                ...
                [RegularExpression("(^\\d{4}[0-9Xx]{1}$)|(^\\d{4}-[0-9Xx]{1}$|(^\\d{4})|(^\\d{3}))", ErrorMessage = "Parameter :: bank_branch :: must be between 3-6 character length characters length.#LENGTH")]
                public string bank_branch { get; set; }

                [JsonProperty(Order = 17)]
                [Required(ErrorMessage = "Parameter :: beneficiary_account_number :: is required.#REQUIRED")]
                [RegularExpression("(^\\d{4,12}[0-9Xx]{1}$)|(^\\d{4,12}-[0-9Xx]{1}$)", ErrorMessage = "Parameter :: beneficiary_account_number :: has minimum 5 characters and a maximum of 14.#LENGTH")]
